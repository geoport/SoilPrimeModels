# Makefile for SoilPrimeModels project
...
help: ## Show this help message
	...
	@echo "  test          - Run Python tests for SoilPrimeModels"
	...
	@echo "  install       - Install SoilPrimeModels in development mode"
	@echo "  lint          - Run linting on SoilPrimeModels code"
	@echo "  format        - Format SoilPrimeModels code"
...
test: ## Run Python tests for SoilPy
	@echo "$(GREEN)Running SoilPrimeModels tests...$(NC)"
	@if [ ! -d "venv" ]; then \
...
test-verbose: ## Run SoilPrimeModels tests with verbose output
	@echo "$(GREEN)Running SoilPrimeModels tests (verbose)...$(NC)"
	@source venv/bin/activate && \
	PYTHONPATH=src python -m pytest tests/ -v -s --tb=long

test-quick: ## Run SoilPrimeModels tests quickly (no verbose output)
	@echo "$(GREEN)Running SoilPrimeModels tests (quick)...$(NC)"
	@source venv/bin/activate && \
	PYTHONPATH=src python -m pytest tests/ --tb=no -q

test-failed: ## Run only failed SoilPrimeModels tests
	@echo "$(GREEN)Running failed SoilPrimeModels tests...$(NC)"
	@source venv/bin/activate && \
	PYTHONPATH=src python -m pytest tests/ --lf -v

setup: ## Setup Python virtual environment and dependencies
	@echo "$(YELLOW)Setting up SoilPrimeModels environment...$(NC)"
	@python3 -m venv venv && \
	source venv/bin/activate && \
...
install: ## Install SoilPrimeModels in development mode
	@echo "$(YELLOW)Installing SoilPrimeModels in development mode...$(NC)"
	@source venv/bin/activate && \
	pip install -e .

lint: ## Run linting on SoilPrimeModels code
	@echo "$(YELLOW)Running linting on SoilPrimeModels...$(NC)"
	@source venv/bin/activate && \
	flake8 src/ tests/ --max-line-length=100 --ignore=E203,W503

format: ## Format SoilPrimeModels code
	@echo "$(YELLOW)Formatting SoilPrimeModels code...$(NC)"
	@source venv/bin/activate && \
...
status: ## Check project status
	@echo "$(BLUE)Checking SoilPrimeModels project status...$(NC)"
	@if [ -d "venv" ]; then \
...
# Package management
build: ## Build package
	@echo "$(GREEN)Building SoilPrimeModels package...$(NC)"
	@source venv/bin/activate && \
	python -m build

publish: ## Publish package to PyPI (requires credentials)
	@echo "$(GREEN)Publishing SoilPrimeModels package...$(NC)"
	@source venv/bin/activate && \
